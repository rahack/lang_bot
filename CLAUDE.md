...
Production-хостинг — **Northflank** (Sandbox free tier, регион **US Central**), деплой из GitHub-репо `rahack/lang_bot` через `Dockerfile` на push в `master`. Регион US важен: Gemini-fallback заблокирован в EEA. Реплик ровно одна — иначе конфликт Telegram polling. Подробности и переменные окружения — в `README.md`. Fly.io пробовали раньше, отказались.
